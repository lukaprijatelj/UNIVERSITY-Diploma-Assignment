			...
			float fresnelSchlick(float f0, vec3 l, vec3 h) 
			{
				float LoH = l.dot(h);
			...
			float NDFGGX(float a, vec3 n, vec3 h, float NoH) {
				float a2 = a*a;
				return a2 / (4.0 * pow(pow(NoH, 2.0) * (a2 - 1.0) + 1.0, 2.0));
			}
			float GCookTorrance(float a, vec3 l, vec3 v, vec3 h, vec3 n, float NoL, float NoV) 
			{

...
namespace.html.Canvas = (() => {
    let Canvas = function (resizable) {
        resizable = typeof resizable != 'undefined' ? resizable : false;
    ...
    Canvas.prototype.getPixel = function (posX, posY) {
        var _this = this;
        return _this.toImageData(posX, posY, 1, 1).data;
    };
...
namespace.html.Div = (() => {
    let Div = function (classType) {
        let _this = document.createElement('div');
        if (classType) {
            _this.addClass(classType);
        }
...
HTMLElement.prototype.removeClassStartingWith = function (value) {
    let expression = new RegExp('\\b' + value + '\\S+', 'g');
    this.className = this.className.replace(expression, '');
    return this;
};
...
HTMLElement.DEBUG = function (element, text) {
    element.addClass('debug');
    element.setAttribute('debug-text', text);
};
...
Element.prototype.remove = function () {
    ...
    if (parentElement) {
        ...
        parentElement.updateVisibleChilds();
    }
};
...
HTMLElement.prototype.setAutoloadSrc = function (url) {
    let _this = this;
    let ATTR_NAME = 'autoload-src';
    _this.setAttribute(ATTR_NAME, url);
};
HTMLElement.prototype.updateVisibleChilds = function () {
    let _this = this;
    if (_this.hasJsClass('track-visible-childs') == false) {
        return;
    }
    let parent = _this;
    let children = parent.children;
    let expression;
    let CHILD_COUNT_CLASSNAME = 'child-count-';
    parent.removeClassStartingWith(CHILD_COUNT_CLASSNAME + '\\d');
    let NTH_CHILD_CLASSNAME = 'child-';
    let FIRST_CHILD_CLASSNAME = 'child-first';
    let LAST_CHILD_CLASSNAME = 'child-last';
    expression = new RegExp('\\b' + NTH_CHILD_CLASSNAME + '\\d\\S+|' + FIRST_CHILD_CLASSNAME + '|' + LAST_CHILD_CLASSNAME, 'g');
    for (let i = 0; i < children.length; i++) {
        let current = children[i];
        current.className = current.className.replace(expression, '');
    }
    let firstChild = null;
    let lastChild = null;
    let count = 0;
    for (let i = 0; i < children.length; i++) {
        ...
        if (current.isHidden() == false) {
            if (!firstChild) {
                firstChild = current;
                current.addClass(FIRST_CHILD_CLASSNAME);
            }
            current.addClass(NTH_CHILD_CLASSNAME + count);
            lastChild = current;
            count++;
        }
    }
    if (lastChild) {
        lastChild.addClass(LAST_CHILD_CLASSNAME);
    }
    parent.addClass(CHILD_COUNT_CLASSNAME + count);
};
HTMLElement.prototype.trackVisibleChilds = function () {
    let _this = this;
    _this.addJsClass('track-visible-childs');
    _this.updateVisibleChilds();
};
HTMLElement.prototype.hide = function () {
    let _this = this;
    _this.addClass('hidden');
    if (_this.parentElement) {
        _this.parentElement.updateVisibleChilds();
    }
};
...
namespace.html.Inline = (() => {
    let Inline = function (classType) {
        if (!classType) {
            new Exception.Other('Element must always contain type class!');
            return;
        }
        let _this = document.createElement('inline');
        _this.addClass(classType);
        Object.cloneData(_this, Inline.prototype);
        return _this;
    };
    Inline.prototype.setAspectRatio = function (ratioW, ratioH) {
        let _this = this;
        let aspectRatioWrapper = _this.querySelector('wrapper_.aspectRatio');
        if (!aspectRatioWrapper) {
            aspectRatioWrapper = new namespace.html.Wrapper();
...
namespace.html.ReservedSpace = (() => {
    let ReservedSpace = function (classType) {
        if (!classType) {
            new Exception.Other('Element must always contain type class!');
            return;
        }
